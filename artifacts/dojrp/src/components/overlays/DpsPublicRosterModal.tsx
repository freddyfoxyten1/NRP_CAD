...
export default function DpsPublicRosterModal({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([rosterRows, rankRows, vehicleRows, equipmentRows]) => {
        if (cancelled) return;
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter(m =>
      [m.username, m.discord_username, memberRank(m), m.callsign, m.group_name]
        .filter(Boolean)
  ...
  const groupedMembers = useMemo(() => {
    const groups = new Map<string, { sort: number; members: RosterMember[] }>();
    filteredMembers.forEach(m => {
      const label = (m.group_name?.trim() || "Personnel");
      const existing = groups.get(label);
      if (existing) existing.members.push(m);
      else groups.set(label, { sort: Number(m.group_sort_order ?? 999), members: [m] });
    });
    return [...groups.entries()]
      .sort((a, b) => a[1].sort - b[1].sort || a[0].localeCompare(b[0]))
      .map(([label, data]) => ({ label, members: data.members }));
  }, [filteredMembers]);
  ...
  const tabs: { id: RosterTab; label: string; icon: typeof Users; count: number }[] = [
    { id: "personnel", label: "Personnel", icon: Users, count: members.length },
