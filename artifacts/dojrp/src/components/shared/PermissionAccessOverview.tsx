...
import { ChevronDown, ChevronUp, Lock, Search, Shield } from 'lucide-react';
...
type PermissionAccessOverviewProps = {
  ...
  emptyMessage?: string;
  /** When true, the card header toggles body visibility. Defaults to true. */
  collapsible?: boolean;
          ...
          {collapsible ? (
            ...
            >
              <span className="min-w-0 flex-1">
                <span className={`flex items-center gap-2 text-sm font-black uppercase tracking-[0.2em] ${accentTextClass}`}>
                  {title}
                  {collapsed
                    ? <ChevronDown className="h-4 w-4 shrink-0 opacity-60 transition-transform group-hover:opacity-100" />
                    : <ChevronUp className="h-4 w-4 shrink-0 opacity-60 transition-transform group-hover:opacity-100" />}
                </span>
