...
type DphModernShellProps = {
  ...
  onSupervisoryPanel: () => void;
  canSeeDepartmentPanel: boolean;
  onDepartmentPanel: () => void;
  ...
  return (
    <>
      ...
      <div className="sidebar-scroll min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 pb-2 lg:px-3">
        <p className="mb-1 mt-1 px-3 text-[9px] font-black uppercase tracking-[0.24em] text-[#3f5470]">
          Department sections
        </p>
      ...
      <div className="shrink-0 border-t border-[#132033]">
        {showPanelTools && (
          <div className="px-4 py-4">
            <p className="mb-2 px-2 text-[9px] font-black uppercase tracking-[0.24em] text-[#3f5470]">
              Department tools
            </p>
            <div className="space-y-0.5">
              ...
              {canSeeDepartmentPanel && (
                <button
                  type="button"
                  onClick={() => { onDepartmentPanel(); finish(); }}
                  className={`flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-xs font-bold transition-colors ${
        ...
        <div className={`px-4 py-3 space-y-0.5 ${showPanelTools ? 'border-t border-[#132033]' : ''}`}>
          <button
            type="button"
            onClick={() => { navigate('/portal_dashboard'); finish(); }}
...
export function DphModernShell({
  ...
  const bottomNav = [
    { id: 'information', label: 'Info', icon: tabs.find(t => t.id === 'information')?.icon ?? Home },
    { id: 'personnel-roster', label: 'Roster', icon: tabs.find(t => t.id === 'personnel-roster')?.icon ?? Home },
        ...
        <section className="flex min-h-full flex-1 flex-col lg:ml-[300px]">
          <header className="sticky top-0 z-40 flex items-center justify-between gap-3 border-b border-[#132033]/80 bg-[#030810]/90 px-4 py-3 backdrop-blur-xl sm:px-5 lg:px-8 lg:py-4">
            ...
            <div className="hidden lg:block">
              <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#4a6080]">Department</p>
              ...
              {bottomNav.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (id === 'menu') setMobileNavOpen(true);
