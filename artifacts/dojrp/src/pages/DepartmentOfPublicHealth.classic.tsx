...
const DepartmentOfPublicHealth = ({ shellTheme = 'classic' }: { shellTheme?: DphShellTheme } = {}) => {
  ...
  const [indexInfoForm,    setIndexInfoForm]    = useState<IndexInfoFormState>(emptyIndexInfoForm);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const showLoading = roster.length === 0;
    ...
    void (async () => {
      ...
      try {
        ...
        if (!cancelled && showLoading) setRosterLoading(false);
      }
      try {
  ...
  const fetchPageInfo = () => {
    ...
    fetch('/api/dph/content/page_info')
      ...
      .catch(() => {})
      .finally(() => { if (showLoading) setInfoLoading(false); });
  };
  ...
  const rosterDiscordIds = useMemo(() => roster.map(m => m.discord_id), [roster]);
  ...
  // Only block the shell on session bootstrap or a tab's first load (no cached data).
  // Refetches keep the current view mounted so tab switches do not flash a full-page loader.
  const pageLoading = isLoading || (
    activeTab === 'personnel-roster' ? (rosterLoading && roster.length === 0)
    : activeTab === 'division-roster' || activeTab === 'divisions-information' ? (rosterLoading && roster.length === 0)
    : activeTab === 'vehicle-roster' ? (fleetLoading && fleet.length === 0)
    : activeTab === 'equipment-roster' ? (equipmentLoading && equipment.length === 0)
    : activeTab === 'event-calendar' ? (eventsLoading && events.length === 0)
    : activeTab === 'information' ? (infoLoading && pageInfo === null)
    : activeTab === 'resources' ? (resourcesLoading && resources.length === 0)
    : activeTab === 'department-panel' ? (
      panelSection === 'personnel' ? (panelLoading && panelMembers.length === 0)
      : panelSection === 'division' ? false
      : panelSection === 'vehicle' ? ((fleetLoading || categoriesLoading) && fleet.length === 0)
      : panelSection === 'equipment' ? ((equipmentLoading || eqCategoriesLoading) && equipment.length === 0)
      : panelSection === 'resources' ? (resourcesLoading && resources.length === 0)
      : panelSection === 'calendar' ? (eventsLoading && events.length === 0)
      : panelSection === 'information' ? (infoLoading && pageInfo === null)
      : false
    )
    : false
  );
      ...
      <div className="flex min-h-screen flex-col pt-[53px] lg:flex-row lg:pt-0">
        ...
        <aside className={`border-b border-[#131f30] bg-[#02060b] px-5 py-5 lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:w-[265px] lg:flex-col lg:border-b-0 lg:border-r lg:border-[#131f30] lg:transition-transform lg:duration-300 ${sidebarOpen ? 'lg:translate-x-0' : 'lg:-translate-x-full'}`}>
          ...
          <div className="sidebar-scroll mt-8 lg:flex-1 lg:overflow-x-hidden lg:overflow-y-auto">
            <p className="mb-2 px-3 text-[9px] font-black uppercase tracking-[0.24em] text-[#3f5470]">Department sections</p>
