  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      try {
        ...
        const next = { ...current, ...data.account };
        setCadSession(next);
        ...
        try {
          const meRes = await fetch(
            `/api/dph/me?username=${encodeURIComponent(next.username)}`,
            { headers: { accept: 'application/json' } },
          );
          if (meRes.ok) {
            const me = await meRes.json() as { dph_rank?: string | null; can_access_iab?: boolean } | null;
            if (me) {
              dphIab = Boolean(me.can_access_iab);
              if (!cancelled) setDphRank(me.dph_rank ?? null);
            }
          }
        } catch { /* non-fatal — fall through to access check */ }
  ...
  if (isLoading || !session) {
    return (
      <main className="min-h-screen bg-[#02060b] text-white">
        <PageLoadingScreen loading label="Loading Internal Affairs…" accent="#34d399" />
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#02060b] text-white">
      <div className="flex min-h-screen flex-col lg:flex-row">
        <aside className="border-b border-[#131f30] bg-[#02060b] px-5 py-5 lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:w-[265px] lg:flex-col lg:border-b-0 lg:border-r lg:border-[#131f30]">
          ...
          <p className="mt-4 text-sm font-black text-[#4384ff]">{session.username}</p>
          <p className="mt-1 text-[10px] font-black uppercase tracking-[0.22em] text-[#526179]">
            {(dphRank || session.staff_rank || session.rank || 'Member').trim()}
          </p>

          <nav className="mt-8 flex flex-col gap-1">
            <button
              type="button"
              className="flex items-center gap-2.5 rounded-md border-l-2 border-[#34d399] bg-[#04140f] px-4 py-3 text-left text-sm font-semibold text-[#34d399]"
            >
        ...
        <section className="flex-1 lg:ml-[265px]">
          <header className="flex items-center justify-between border-b border-[#131f30] px-5 py-4 sm:px-8">
            <div>
              <h1 className="text-xl font-black tracking-[-0.04em] text-white">DPH Internal Affairs</h1>
              <p className="mt-1 text-xs text-[#8392aa]">Department of Public Health · Internal Affairs Bureau</p>
            </div>
            <button
              type="button"
              onClick={() => void handleSignOut()}
...
export default DphInternalAffairs;
